...
The `pivot()` method can be used to pivot a `DataFrame`.  
...
## R base
```{r}
#| label: first-pivot-rbase
reshape(df[,-which(names(df) %in% c("localisation"))], 
        idvar = "country", 
        timevar = "city", 
        direction = "wide")
```
...
```{r}
#| label: first-pivot-polars-tidyr
df |>
  pivot_wider(
    id_cols = country,
    names_from = city,
