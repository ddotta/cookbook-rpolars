...
We can see that Polars has identified that only 2 columns are needed to execute our query (`PROJECT 2/5 COLUMNS`). And Polars did this without me having to select these variables myself (for example with a `select` method).  
**The added value of Polars is that it appliess some optimizations that I/you might not even have known about. 💪**
