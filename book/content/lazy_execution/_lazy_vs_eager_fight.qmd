...
```{r}
#| label: fight-lazy_dataframe

tic()
#| label: fight-eager_dataframe
as_polars_df(fake_data)$select(
  pl$col("region","departement","priority")
)$
  filter(
...
pl$read_csv("Datasets/fakir_file.csv", infer_schema_length=0)$
  select(
    pl$col("region","departement","priority","age"))$
  with_columns(
