...
You can use an `alias` to ensure column names are unique.  
For example:  
...
pl$DataFrame(iris)$
  groupby("Species")$
  agg(
    pl$col(c("Petal.Length"))$sum()$alias("Sum_Petal.Length"),
    pl$col(c("Petal.Length"))$mean()$alias("Mean_Petal.Length"),
    pl$col(c("Petal.Width"))$sum()$alias("Sum_Petal.Width")
    )
```
