...
The `value_counts()` method can be used to count the occurences of uniques values in a Series of a DataFrame. 
`value_counts()` works with a `Series`. It must therefore be supplied with the `select()` method. See [here](#select-columns) to learn about it.
